...
def load_config(config: dict) -> DevWebscraper | WebScraper:
    ...
    if (config["dev_settings"] == "on"):
        ...
        raise Exception("Undefined setting in webscraper_config.yml > dev_settings [\"on\"/\"off\"]")
    
    return new_webscraper

if __name__ == "__main__":
    new_webscraper: DevWebscraper | WebScraper = load_config(yaml.safe_load(open("webscraper_config.yml")))
